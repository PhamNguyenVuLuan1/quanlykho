...
@section('content')
    <div class="card">
        <div class="card-header">Sửa nhập kho</div>
        <div class="card-body">
            <form action="{{ route('nhapkho.sua') }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <label class="form-label" for="nguoinhap_id">Người nhập</label>
                    <select class="form-select @error('nguoinhap_id') is-invalid @enderror" id="nguoinhap_id" name="nguoinhap_id" required>
                        ...
                        @foreach($nguoidung as $value)
                            <option value="{{ $value->id }}" {{ ($nguoidung->nguoinhap_id == $value->id) ? 'selected' : '' }}>{{ $value->name }}</option>
                        @endforeach
                    </select>
                    @error('nguoinhap_id')
                ...
                <label class="form-label" for="sanphamnhap_id">Sản phẩm</label>
                    <select class="form-select @error('sanphamnhap_id') is-invalid @enderror" id="sanphamnhap_id" name="sanphamnhap_id" required>
                        ...
                        @foreach($sanpham as $value)
                            <option value="{{ $value->id }}" {{ ($sanpham->sanphamnhap_id == $value->id) ? 'selected' : '' }}>{{ $value->tensanpham }}</option>
                ...
                <label class="form-label" for="loaisanphamnhap_id">Loại sản phẩm</label>
                    <select class="form-select @error('loaisanphamnhap_id') is-invalid @enderror" id="loaisanphamnhap_id" name="loaisanphamnhap_id" required>
                        ...
                        @foreach($loaisanpham as $value)
                            <option value="{{ $value->id }}" {{ ($loaisanpham->loaisanphamnhap_id == $value->id) ? 'selected' : '' }}>{{ $value->tenloai }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('nhacungcapnhap_id') is-invalid @enderror" id="nhacungcapnhap_id" name="nhacungcapnhap_id" required>
                        ...
                        @foreach($nhacungcap as $value)
                            <option value="{{ $value->id }}" {{ ($nhacungcap->nhacungcapnhap_id == $value->id) ? 'selected' : '' }}>{{ $value->tenncc }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" min="0" class="form-control @error('soluongnhapkho') is-invalid @enderror" id="soluongnhapkho" name="soluongnhapkho" value="{{ $nhapkho->soluongnhapkho }}" required />
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('donvitinh') is-invalid @enderror" id="donvitinh" name="donvitinh" value="{{ $nhapkho->donvitinh }}" required />
                    @error('donvitinh')
                        <div class="invalid-feedback"><strong>{{ $message }}</strong></div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" min="0" class="form-control @error('gianhap') is-invalid @enderror" id="gianhap" name="gianhap" value="{{ $nhapkho->gianhap }}" required />
                    @error('gianhap')
                        <div class="invalid-feedback"><strong>{{ $message }}</strong></div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" min="0" class="form-control @error('thanhtien') is-invalid @enderror" id="thanhtien" name="thanhtien" value="{{ $nhapkho->thanhtien }}" required />
                    @error('thanhtien')
                        <div class="invalid-feedback"><strong>{{ $message }}</strong></div>
                    @enderror
                </div>
