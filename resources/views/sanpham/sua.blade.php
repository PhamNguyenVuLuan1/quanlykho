...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('sanpham.sua') }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <label class="form-label" for="loaisanpham_id">Loại sản phẩm</label>
                    <select class="form-select @error('loaisanpham_id') is-invalid @enderror" id="loaisanpham_id" name="loaisanpham_id" required>
                        ...
                        @foreach($loaisanpham as $value)
                            <option value="{{ $value->id }}" {{ ($loaisanpham->loaisanpham_id == $value->id) ? 'selected' : '' }}>{{ $value->tenloai }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('nhacungcap_id') is-invalid @enderror" id="nhacungcap_id" name="nhacungcap_id" required>
                        <option value="">-- Chọn --</option>@foreach($nhacungcap as $value)
                        <option value="{{ $value->id }}"{{ ($sanpham->nhacungcap_id == $value->id) ? 'selected' : '' }}>{{ $value->tenncc }}</option>
                        @endforeach
                    </select>
                    @error('nhacungcap_id')
                        <div class="invalid-feedback"><strong>{{ $message }}</strong></div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('donvitinh') is-invalid @enderror" id="donvitinh" name="donvitinh" value="{{ $sanpham->donvitinh }}" required />
                    @error('donvitinh')
                        <div class="invalid-feedback"><strong>{{ $message }}</strong></div>
                    @enderror
                </div>
